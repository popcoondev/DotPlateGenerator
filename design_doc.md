...
- ユーザー塗料パレット管理
  - メニュー「設定」→「パレット設定」で呼び出し
  - 登録済みのパレット色一覧表示（サムネイル付き）
  - ＋ボタンでカラーピッカーから色を追加
  - －ボタンで選択色を削除
  - 設定は永続化 (QSettings利用)
...
- 色レイヤーモードを有効化するチェックボックス
- 「レイヤーを更新」ボタンで最新ドットデータを反映
- 明度・彩度昇順/降順ソートボタン
- ドラッグ＆ドロップおよび▲▼ボタンでレイヤー順序変更
- 各行項目:
  - ドットカラーのサムネイル
  - 登録パレットからの混色比サムネイル (パレットアイコンを複数並べる)
  - 高さ設定用スピンボックス (0.0～10.0)
  - 順序移動ボタン (▲▼)
...
1. 元の画像の色をそのまま使用（量子化なし）
2. 高画質・詳細な画像やマンガ絵に最適
3. 使用色数が多くなるため、モデルのファイルサイズは大きくなる可能性あり
...
- 概要: OpenAI APIを利用し、指定した領域や色をもとにピクセルデータを自動生成するインテリジェントブラシ機能。
- ワークフロー:
  1. ユーザーが「AIブラシ」モードを選択し、対象のドットをクリック。
  2. 対象の色情報および周辺のピクセルデータをプロンプトに組み込み、ChatCompletion API(chat.completions)へリクエストを送信。
  3. モデルからの応答をMarkdownのコードブロック形式で受け取り、JSONまたはリスト形式のピクセルデータを抽出。
     - 応答テキストからコードフェンスを除去
     - バランス括弧マッチングで先頭のリスト(`[]`)またはオブジェクト(`{}`)部分を抽出
  4. 抽出したテキストを`json.loads`でパースし、失敗時は`ast.literal_eval`でリテラル評価を試行
  5. NumPy配列に変換してピクセルデータを更新し、プレビューを再描画。
- APIキー設定:
  - 設定ダイアログまたは環境変数`OPENAI_API_KEY`から読み込む
- エラー処理:
  - JSONデコードやリテラル評価で例外発生時にはキャッチし、エラーメッセージをメッセージボックスまたはステータスバーに表示
