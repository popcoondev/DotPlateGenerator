...
class STLPreviewDialog(QDialog):
    def __init__(self, stl_path, parent=None):
        ...
        # 別スレッドでSTLを読み込み
        threading.Thread(target=self.load_stl_preview, args=(stl_path,)).start()
    
    def load_stl_preview(self, stl_path):
        try:
            ...
            # メインスレッドでのGUI更新を要求
            QApplication.instance().postEvent(self, MeshLoadedEvent(mesh))
    ...
    def export_stl(self):
        ...
        if out_path:
            ...
            try:
                ...
                self.input_label.setText("STLファイルを生成中...")
